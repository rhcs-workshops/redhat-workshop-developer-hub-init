...
- A continuación, verás formulario en el que sólo tendrás que modificar el campo *Name* con tu nombre y apellidos *en minúscula* y usando el guión (`-`) como separador de espacios.
...
- Mantén el resto de valores por defecto y haz clic en el botón *Next*.

- En el tercer y último formulario, verás valores predeterminados para el servidor GitLab que alojará los repositorios de código fuente y configuración de tu aplicación. Mantén los valores por defecto y haz clic en el botón *Review* para continuar.

- Se te presentará un resumen. Ahora puedes revisar que todo esté en orden y haz clic en el botón *Create*.

- El Software template ha finalizado, ahora veremos que ha generado, continúa haciendo clic en el enlace *Open Component in catalog*.
